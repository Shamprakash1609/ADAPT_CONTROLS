...
document.addEventListener('DOMContentLoaded', () => {
    ...
    mapInput.addEventListener('input', () => {
        const ipAddress = mapInput.value;
        
    });

    mapInput.addEventListener('keypress', (event) => {
        if (event.key === 'Enter') {
            const ipAddress = mapInput.value.trim();
            if (ipAddress) {
                cameraFeed.src = `http://${ipAddress}:8080/video`;
            }
        }
    });

    switchButton.addEventListener('click', () => {
        currentView = currentView === 'camera' ? 'map' : 'camera';
    ...
    navButton.addEventListener('click', () => {
        window.open('full-map.html', '_blank');
    });
    ...
    function sendGoal(x, y) {
        const goalMsg = {
            ...
            pose: {
                ...
                orientation: { x: 0.0, y: 0.0, z: 0.0, w: 1.0 } // Default orientation facing forward
            }
        };
        ...
        .then(response => {
            if (!response.ok) {
                console.error('Failed to send goal');
            }
        })
        .catch(error => {
            console.error('Error:', error);
